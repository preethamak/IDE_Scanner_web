export type QueueDecision = {
  ...
  updated_at: string;
  scans?: Record<string, unknown> | Array<Record<string, unknown>> | null;
};

export type DecisionQueue = {
  open: QueueDecision[];
  dueSoon: QueueDecision[];
  resolved: QueueDecision[];
};

export function groupDecisionQueue(
  ...
  for (const decision of decisions) {
    if (decision.resolved_at) result.resolved.push(decision);
      ...
      decision.due_at &&
      new Date(decision.due_at).getTime() <= threshold
    )
      result.dueSoon.push(decision);
