...
export function emailDeliveryConfigured() { return Boolean(runtimeEnv("RESEND_API_KEY") && runtimeEnv("NOTIFICATION_FROM_EMAIL")); }
export function emailPayload(alert: Record<string, unknown>, recipient: string) {
  ...
  if (metadata.release_event === true) {
    ...
    return { from: runtimeEnv("NOTIFICATION_FROM_EMAIL"), to: [recipient], subject: `[GuardRails] Release change: ${extension}@${version}`, text: `${summary}\n\nReviewed baseline: ${extension}@${baseline}\nNew artifact: ${extension}@${version}\nState: ${state.replaceAll("_", " ")}\n\nOpen evidence: ${site}/extensions/${encodeURIComponent(extension)}/versions/${encodeURIComponent(version)}` };
  }
  return { from: runtimeEnv("NOTIFICATION_FROM_EMAIL"), to: [recipient], subject: `[GuardRails] ${severity}: ${String(alert.title || "Monitoring alert")}`, text: `${String(alert.summary || "")}\n\nArtifact: ${extension}@${version}\nOpen evidence: ${site}/extensions/${encodeURIComponent(extension)}/versions/${encodeURIComponent(version)}` };
}
