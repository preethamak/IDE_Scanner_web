...
export async function POST(request: Request) {
  try {
    ...
    const db = publicDb(); if (!db) throw new Error("Submissions are temporarily unavailable."); const requester = request.headers.get("x-forwarded-for")?.split(",")[0] || "unknown"; const hash = createHash("sha256").update(`${process.env.SCAN_RATE_LIMIT_SECRET || "ide-scanner"}:${requester}`).digest("hex");
