...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await serviceDb().from("team_monitoring_alerts").select("*").eq("team_id", id).in("state", ["unread", "read", "acknowledged"]).order("created_at", { ascending: false }).limit(100);
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const patch = { state, read_at: state === "read" ? now : null, resolved_at: state === "acknowledged" || state === "dismissed" ? now : null };
    const { data, error } = await serviceDb().from("team_monitoring_alerts").update(patch).eq("id", alertId).eq("team_id", id).select("id,state,read_at,resolved_at").maybeSingle();
    if (error) throw error;
