...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (provider === "cloudflare") return NextResponse.json({ invitations: (await getWorkspaceState(id)).invitations });
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (provider === "cloudflare") {
      ...
      const invitation = { id: newId(), role, expires_at: new Date(Date.now() + expiresInDays * DAY).toISOString(), accepted_at: null, created_at: nowIso(), token_hash: tokenHash(token) };
...
export async function DELETE(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (provider === "cloudflare") {
      ...
      state.invitations = state.invitations.filter((invitation) => String(invitation.id) !== invitationId);
      if (before === state.invitations.length) return NextResponse.json({ error: "Pending invitation not found." }, { status: 404 });
      await saveState(id, state);
