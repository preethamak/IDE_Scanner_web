...
import { ArrowRight, Check, Cloud, Code2, HardDrive, LockKeyhole, PackageCheck, ScanSearch, ShieldCheck, TerminalSquare } from "lucide-react";
import CliInstallCommand from "@/app/CliInstallCommand";
import "./cli.css";

export const metadata: Metadata = {
  title: "Guardrails CLI",
  description: "Scan extensions already installed in your IDE without uploading or executing their code.",
};
...
export default function CliPage() {
  return <main className="cliPage">
    <section className="cliHero">
      <div className="cliHeroCopy">
        <span className="cliEyebrow"><TerminalSquare /> Guardrails CLI</span>
      ...
      <div className="cliTerminal" aria-label="Guardrails CLI terminal preview">
        ...
        <div className="cliTerminalBody">
          <p><em>$</em> guardrails</p>
          <strong>Installed extensions <small>143 detected</small></strong>
          <div className="cliIdeRows"><span><i /> Cursor <b>40</b></span><span><i /> VS Code <b>81</b></span><span><i /> Windsurf <b>22</b></span></div>
          <p><em>›</em> Select an extension to inspect</p>
          <div className="cliResult"><span>ANALYSIS COMPLETE</span><strong>Decision: review</strong><small>Open findings, coverage, and report export.</small></div>
        </div>
      </div>
    </section>

    <section className="cliDecision" aria-labelledby="which-scan">
      <header><span className="cliEyebrow"><PackageCheck /> Choose the right boundary</span><h2 id="which-scan">Two ways to inspect an extension. One evidence standard.</h2><p>Choose the product based on where the extension is. The analysis boundary stays clear.</p></header>
      <div className="cliCompareGrid">
        {comparison.map(({ icon: Icon, label, title, body, use, action, href }) => <article key={label}>
          <Icon />
          <span>{label}</span>
          <h3>{title}</h3>
          <p>{body}</p>
      ...
      <p className="cliEquivalence"><ShieldCheck /> For the same artifact, a properly configured CLI Deep Scan uses the same analysis boundary as website Deep Scan.</p>
    </section>
    ...
    <section className="cliStart" id="install">
      <div className="cliStartIntro"><span className="cliEyebrow"><ScanSearch /> First local scan</span><h2>Install once. Start with the extensions you already trust.</h2><p>The interactive command groups installed extensions by IDE, lets you search before scanning, and produces findings, risk scoring, coverage, and exportable reports.</p></div>
      <ol className="cliSteps">
        <li><span>01</span><div><strong>Install the CLI</strong><code>pipx install guardlens</code></div></li>
        <li><span>02</span><div><strong>Open the local scanner</strong><code>guardrails</code></div></li>
        <li><span>03</span><div><strong>Use Deep Scan when needed</strong><code>{'pipx install "guardlens[analysis]"'}</code></div></li>
      </ol>
      <div className="cliStartActions"><CliInstallCommand /><Link href="/settings">Read analysis boundaries <ArrowRight /></Link></div>
    </section>
  </main>;
}
