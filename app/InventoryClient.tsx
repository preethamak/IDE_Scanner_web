...
function SeverityMeter({ value, severity }: { value: number; severity: string }) {
  const score = Math.max(0, Math.min(100, value)); const angle = -90 + score * 1.8;
  return <span className={`severityMeter severity-${severity.toLowerCase()}`} aria-label={`${severity} severity, risk ${score} out of 100`}><i/><b style={{ transform: `rotate(${angle}deg)` }}/></span>;
}
