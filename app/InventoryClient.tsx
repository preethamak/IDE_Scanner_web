...
export default function InventoryClient({ inventory }: { inventory: PublicInventory }) {
  ...
  return <>
    ...
    <div className="inventoryCards" aria-live="polite">{rows.map((item) => { const risk = Math.max(0, Math.min(100, Math.max(item.risk_score, item.malware_score))); return <Link className="inventoryCard" key={item.scan_id} href={`/extensions/${encodeURIComponent(item.extension_id)}/versions/${encodeURIComponent(item.version)}/scans/${encodeURIComponent(item.scan_id)}`} aria-label={`Open ${item.display_name} Analysis Report`}><ExtensionIcon iconUrl={item.icon_url} publisher={item.publisher} name={item.display_name}/><b className={`inventorySeverity severity-${item.severity.toLowerCase()}`}>{item.severity === "INFO" ? "Informational" : item.severity}</b><div><strong>{item.display_name}{item.publisher_verified ? <BadgeCheck aria-label="Verified Marketplace publisher"/> : null}</strong><code>{item.extension_id}@{item.version}</code><span className={`inventoryDecision ${item.decision}`}>{decisionLabel[item.decision]}</span><p>{item.description || item.publisher}</p><span className="inventoryRiskMeter" aria-label={`Risk index ${risk} of 100`}><i className={riskTone(risk)} style={{ width: `${risk}%` }}/></span></div><span className="inventoryCardGo">Analysis Report <ArrowUpRight/></span></Link>; })}</div>
