...
import { ArrowUpRight, BadgeCheck, Filter, Search, ShieldCheck } from "lucide-react";
...
export default function InventoryClient({ inventory }: { inventory: PublicInventory }) {
  ...
  return <>
    ...
    <div className="inventoryCards" aria-live="polite">{rows.map((item) => <Link className="inventoryCard" key={item.extension_id} href={`/extensions/${encodeURIComponent(item.extension_id)}/versions/${encodeURIComponent(item.version)}`} aria-label={`Open ${item.display_name} report`}><ExtensionIcon iconUrl={item.icon_url} publisher={item.publisher} name={item.display_name}/><b className={`inventorySeverity severity-${item.severity.toLowerCase()}`}>{item.severity === "INFO" ? "Informational" : item.severity}</b><div><strong>{item.display_name}{item.publisher_verified ? <BadgeCheck aria-label="Verified Marketplace publisher"/> : null}</strong><code>{item.extension_id}@{item.version}</code><p>{item.description || item.publisher}</p></div><ArrowUpRight/></Link>)}</div>
