...
export default function InventoryClient({ inventory }: { inventory: PublicInventory }) {
  ...
  return <>
    ...
    <div className="inventoryCards" aria-live="polite">{rows.map((item) => { const risk = Math.max(0, Math.min(100, Math.max(item.risk_score, item.malware_score))); return <Link className="inventoryCard" key={item.scan_id} href={`/extensions/${encodeURIComponent(item.extension_id)}/versions/${encodeURIComponent(item.version)}/scans/${encodeURIComponent(item.scan_id)}`} aria-label={`Open ${item.display_name} Deep Scan`}><ExtensionIcon iconUrl={item.icon_url} publisher={item.publisher} name={item.display_name}/><span className="inventorySignals"><b className={`inventoryDecision ${item.decision}`}>{decisionLabel(item.decision)}</b><small className={`inventorySeverity severity-${item.severity.toLowerCase()}`}>{item.severity === "INFO" ? "Informational" : item.severity}</small></span><div><strong>{item.display_name}{item.publisher_verified ? <BadgeCheck aria-label="Verified Marketplace publisher"/> : null}</strong><code>{item.extension_id}@{item.version}</code><p>{item.description || item.publisher}</p><span className="inventoryRiskMeter" aria-label={`Risk index ${risk} of 100`}><i className={riskTone(risk)} style={{ width: `${risk}%` }}/></span></div><span className="inventoryCardGo">Deep Scan <ArrowUpRight/></span></Link>; })}</div>
...
function decisionLabel(value: string) { return value === "allow" ? "Allow" : value === "review" ? "Review" : value === "block" ? "Block" : "No decision"; }
