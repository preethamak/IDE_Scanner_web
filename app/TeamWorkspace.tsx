    ...
    <div className={styles.app}>
      ...
      <main className={styles.main}>
        ...
        <div className={styles.content}>
          ...
          {view === "activity" ? (
            <ActivityView alerts={alerts} decisions={decisions} />
          ) : null}
          {view === "settings" ? (
            <SettingsView
              team={activeTeam}
      ...
      <section className={styles.timeline}>
        ...
        {decisions.slice(0, 10).map((item) => (
          <article key={item.id}>
            ...
            <div>
              <strong>{humanize(item.decision)} decision recorded</strong>
              <p>
                {item.extension_id} release @{item.version}
              </p>
            </div>
            <time>Decision</time>
          </article>
        ))}
        {!alerts.length && !decisions.length ? <EmptyReview /> : null}
      </section>
    </>
  );
}
