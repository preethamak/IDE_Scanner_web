...
type Delivery = { status: string; attempts: number; delivered_at: string | null; last_error: string | null };
...
type Channel = { id: string; kind: "slack_webhook" | "generic_webhook"; label: string; minimum_severity: string; last_validated_at: string | null };
...
export default function TeamWorkspace({ initialExtension = "", focus = "workspace" }: { initialExtension?: string; focus?: "workspace" | "monitor" }) {
  ...
  async function connectChannel(event: FormEvent) {
    ...
    const response = await fetch(`/api/teams/${encodeURIComponent(activeTeamId)}/notification-channels`, { method: "POST", headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" }, body: JSON.stringify({ kind: channelKind, label: channelLabel, webhook_url: webhook, minimum_severity: "MEDIUM" }) });
    const body = await response.json();
