...
import { BellRing, LayoutDashboard, Plus, Radar, RefreshCw, Trash2, UserRound, Users } from "lucide-react";
...
type WorkspaceView = "overview" | "monitoring" | "people" | "delivery";
...
export default function TeamWorkspace({ initialExtension = "", focus = "workspace" }: { initialExtension?: string; focus?: "workspace" | "monitor" }) {
  ...
  const [activeView, setActiveView] = useState<WorkspaceView>(focus === "monitor" ? "monitoring" : "overview");
  ...
  const editable = Boolean(activeTeam && ["owner", "admin", "analyst"].includes(activeTeam.role));
  const canManageTeam = Boolean(activeTeam && ["owner", "admin"].includes(activeTeam.role));
  return <section className="workspaceSection teamWorkspace">
    ...
    <p className="sectionIntro">Review an exact release once, then return only when a monitored version changes. Owners, evidence, decisions, and delivery history stay together.</p>
