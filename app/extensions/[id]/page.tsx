...
export default async function ExtensionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="productPage">
    <section className="packageHeader">
      ...
      <div className="packageActions">{scan ? <><Link className="button buttonDark" href={`/extensions/${encodeURIComponent(product.extension.id)}/versions/${encodeURIComponent(version)}`}>Open Deep Scan report</Link><DeepScanButton extensionId={product.extension.id} version={version}/></> : <DeepScanButton extensionId={product.extension.id} version={version}/>}<WatchExtension extensionId={product.extension.id}/><a className="button buttonQuiet" href={`vscode:extension/${product.extension.id}`}>Install <Download size={16}/></a></div>
    </section>
    ...
    <div className="packageLayout">
      <div className="packageMain">
        ...
        <section id="versions" className="productSection"><div className="productSectionHead compact"><span>Release history</span><h2>Published versions</h2><p>Each severity belongs to an immutable version and artifact hash.</p></div><div className="versionTable"><div className="versionHead"><span>Version</span><span>Published</span><span>Severity</span><span/></div>{product.versions.map((item) => <div className="versionRow" key={String(item.version)}><strong>{String(item.version)}</strong><span>{formatDate(item.published_at ? String(item.published_at) : null)}</span><span className={`scanState severityState ${String(item.severity || "not_assessed").toLowerCase()}`}>{versionSeverity(item)}</span><Link href={`/extensions/${encodeURIComponent(product.extension.id)}/versions/${encodeURIComponent(String(item.version))}`}>Open <ChevronRight size={15}/></Link></div>)}</div></section>
...
function versionSeverity(version: Record<string, unknown>): string { const severity = String(version.severity || "").toUpperCase(); return severity === "INFO" ? "INFORMATIONAL" : ["CRITICAL", "HIGH", "MEDIUM", "LOW"].includes(severity) ? severity : "NOT ASSESSED"; }
