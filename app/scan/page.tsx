...
type DeepJob = { id?: string; scan_id?: string; status: string; extension_id?: string; version?: string; error?: string };
...
function Scanner() {
  ...
  useEffect(()=>{if(!deepJob?.id||!["queued","running"].includes(deepJob.status))return;const timer=window.setInterval(async()=>{const response=await fetch(`/api/deep-scans/${deepJob.id}`,{cache:"no-store"});const body=await response.json();if(response.status===401){window.clearInterval(timer);setDeepJob(null);window.location.href=`/account?next=${encodeURIComponent("/scan")}`;return}if(!response.ok||body.status==="failed"){setDeepJob({...body,status:"failed",error:body.error||"Deep Scan failed."});window.clearInterval(timer);return}setDeepJob(body);if(["complete","incomplete"].includes(body.status)){window.clearInterval(timer);window.location.href=`/extensions/${encodeURIComponent(body.extension_id)}/versions/${encodeURIComponent(body.version)}`}},2500);return()=>window.clearInterval(timer)},[deepJob]);
  ...
  async function requestDeepScan(){if(!selected){setError("Select an extension first.");return}setError("");setDeepJob({status:"queued"});const chosen=results.find(item=>item.extension_id===selected);try{const response=await fetch("/api/deep-scans",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({extension_id:selected,version:chosen?.version})});const body=await response.json();if(response.status===401){window.location.href=`/account?next=${encodeURIComponent(`/scan?q=${selected}`)}`;return}if(!response.ok)throw new Error(body.error||"Deep Scan is temporarily unavailable.");if(body.status==="complete"){window.location.href=`/extensions/${encodeURIComponent(selected)}/versions/${encodeURIComponent(chosen?.version||"")}`;return}setDeepJob(body)}catch(cause){setDeepJob(null);setError(cause instanceof Error?cause.message:"Deep Scan is temporarily unavailable.")}}
