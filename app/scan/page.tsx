...
function ResultPanel({ result, level }: { result: DisplayResult; level: string }) {
  ...
  return <section className="scanResult"><div className="resultMast"><div><span className="kicker">{deep ? "Deep Scan · canonical engine" : "Quick Scan · hosted static"}</span><h2>{result.name}</h2><code>{result.extensionId}@{result.version}</code></div><b className={`decision ${result.decision}`}>{result.decision.toUpperCase()}</b></div><div className="resultReason"><ShieldCheck size={22}/><div><strong>{result.reason}</strong><span>Decision applies to artifact <code>{result.hash ? result.hash.slice(0, 16) : "hash unavailable"}</code></span></div></div><div className="resultMetrics"><Metric label="Review priority" value={result.risk}/><Metric label="Malware evidence" value={result.malware}/><Metric label="Executable coverage" value={`${result.coverage}%`}/><Metric label="Recorded alerts" value={result.findings.length}/></div><div className="resultColumns"><div><div className="subhead"><h3>Evidence requiring attention</h3><span>{result.findings.length} alerts</span></div><div className="findingList">{result.findings.slice(0, 8).map((finding, index) => <article key={`${finding.rule_id}-${index}`}><span>{finding.severity || "INFO"}</span><div><strong>{finding.evidence_summary || finding.rule_id || "Scanner evidence"}</strong><p>{finding.rule_id || "finding"}</p><small>{finding.evidence_class || "context"}{finding.file_refs?.[0] ? ` · ${finding.file_refs[0]}` : ""}</small></div></article>)}</div></div><div><div className="subhead"><h3>What this scan assessed</h3><span>{deep ? "Complete profile" : "Limited profile"}</span></div><div className="scanScopeSummary"><strong>{deep ? "Deep Scan" : "Quick Scan"}</strong><p>{deep ? "Native rules, JavaScript AST, Semgrep paths, YARA indicators, dependency intelligence and artifact identity." : "Manifest, artifact identity and bounded static patterns. It does not claim Semgrep, YARA or dependency intelligence ran."}</p><div><span>Package code execution</span><b>Never</b></div><div><span>Scope conclusion</span><b>{deep ? "Recorded in evidence" : "Preliminary"}</b></div></div>{!deep ? <div className="scanLimitation"><strong>Not a Deep Scan</strong><p>Use the permanent extension page to queue the full analyzer suite. Missing analyzers are limitations, not negative findings.</p></div> : null}<div className="reportLinkBox"><strong>Permanent version intelligence</strong><p>Open alerts, capabilities, dependencies, files and artifact evidence.</p><Link href={`/extensions/${encodeURIComponent(result.extensionId)}/versions/${encodeURIComponent(result.version)}`}>Open exact version <ArrowRight size={15}/></Link></div></div></div></section>;
}
function Metric({ label, value }: { label: string; value: string | number }) { return <div><span>{label}</span><strong>{value}</strong></div>; }
function CoverageModes() { return <div className="coverageModes"><div><strong>Deep Scan</strong><span>Native + AST + required Semgrep, YARA and dependency intelligence</span></div><div><strong>Quick Scan</strong><span>Manifest + bounded static patterns + artifact identity</span></div></div>; }
