...
function Scanner() {
  ...
  const selectedItem = results.find((item) => item.extension_id === selected);
  ...
  return <main className="scannerPage pageWrap">
    ...
    {mode === "marketplace" ? <section className="scanWorkspace"><div className="scanInputPanel"><span className="panelNumber">01</span><div><span className="kicker">Find an extension</span><h2>Marketplace search</h2><p>Use a product name, exact <code>publisher.extension</code> identifier, VS Marketplace URL, or <code>vscode:extension</code> URI.</p></div><form onSubmit={(event) => { event.preventDefault(); void searchMarketplace(); }}><Search size={18}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="e.g. Vyper Guard or tintinweb.vscode-vyper"/><button className="button buttonDark" disabled={searching}>{searching ? "Searching…" : "Search"}</button></form>{results.length ? <div className="scanSearchResults">{results.slice(0, 8).map((item) => <button key={item.extension_id} className={selected === item.extension_id ? "selected" : ""} onClick={() => setSelected(item.extension_id)}><MarketplaceIcon item={item}/><div><strong>{item.display_name}</strong><code>{item.extension_id} · {item.version}</code></div><small>{formatCompact(item.install_count)} installs</small><ChevronRight size={17}/></button>)}</div> : <div className="searchEmpty">Search results will include VS Marketplace and Open VSX matches.</div>}</div><div className="scanActionPanel"><span className="panelNumber">02</span><div><span className="kicker">Inspect exact release</span>{selectedItem ? <div className="selectedExtension"><MarketplaceIcon item={selectedItem}/><div><h2>{selectedItem.display_name}</h2><code>{selectedItem.extension_id}@{selectedItem.version}</code><p>{selectedItem.publisher_display_name || selectedItem.publisher} · {formatCompact(selectedItem.install_count)} installs</p></div></div> : <><h2>Select a result</h2><p>Choose the release you want to inspect. The scanner records a result for that exact published artifact.</p></>}</div>{selectedItem ? <><DeepScanButton extensionId={selectedItem.extension_id} version={selectedItem.version}/><button className="preflightButton" disabled={isRunning(scan)} onClick={() => void scanMarketplace(selectedItem.extension_id)}>{isRunning(scan)?"Inspecting…":"Run instant preflight"}</button></> : <button className="button buttonDark scanNow" disabled>Select an extension</button>}<CoverageModes /></div></section> : null}
