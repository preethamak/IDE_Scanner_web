...
type Props = { extension: CatalogExtension; version: string; versions: Array<Record<string, unknown>>; scan: Record<string, unknown> | null; fullAnalysisHref?: string };

export default function PublicSecuritySummary({ extension, version, versions, scan, fullAnalysisHref }: Props) {
  ...
  const next = fullAnalysisHref ? `/account?next=${encodeURIComponent(fullAnalysisHref)}` : `/account?next=${encodeURIComponent(`/extensions/${extension.id}/versions/${version}`)}`;
  return <main className="securitySummary">
    ...
    <section className="summaryBody"><div><span className="kicker">What this means</span><h1>{scanned ? "The key information before you install." : "Read the publisher details before you install."}</h1><p>{scanned ? "This is a plain-language summary of the completed analysis for this exact extension version. Technical evidence and workspace actions are available after sign-in." : "Publisher documentation and release history are public. Request a Deep Scan when you need security evidence for this version."}</p></div><aside>{scanned ? <Link className="button buttonDark" href={next}>Sign in for Full Analysis <ArrowRight size={16}/></Link> : <Link className="button buttonDark" href={next}>Sign in to request a Deep Scan <ShieldCheck size={16}/></Link>}<Link className="button buttonQuiet" href={`/extensions/${encodeURIComponent(extension.id)}`}>Read README and releases <FileText size={16}/></Link></aside></section>
