...
export default function ExtensionSearch({ initialQuery = "", onSelect, submitLabel = "Search", compact = false }: Props) {
  ...
  const open = (item: DiscoveryResult) => onSelect ? <button type="button" onClick={() => onSelect(item)}>Choose release <ArrowRight size={15}/></button> : <div className="discoveryActions"><Link className="primary" href={signedIn ? `/analyze?q=${encodeURIComponent(item.extension_id)}` : `/account?next=${encodeURIComponent(`/analyze?q=${encodeURIComponent(item.extension_id)}`)}`}>{signedIn ? "Deep Scan" : "Create workspace to Deep Scan"} <ArrowRight size={15}/></Link><Link href={`/extensions/${encodeURIComponent(item.extension_id)}/versions/${encodeURIComponent(item.version)}`}>Open Analysis Report</Link><Link href={`/monitor?extension=${encodeURIComponent(item.extension_id)}`}>Monitor</Link></div>;
