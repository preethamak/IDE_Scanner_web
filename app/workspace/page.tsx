...
type Alert = { id: string; extension_id: string; version: string; scan_id?: string | null; kind: string; severity?: string | null; state: string; title: string; summary: string; created_at: string; extensions?: Array<{ display_name?: string; icon_url?: string }> | null };
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    const [alertResult, jobResult] = await Promise.all([
      db.from("monitoring_alerts").select("id,extension_id,version,scan_id,kind,severity,state,title,summary,created_at,extensions(display_name,icon_url)").in("state", ["unread", "read", "acknowledged"]).order("created_at", { ascending: false }).limit(30),
  ...
  return <main className="workspacePage dashboardPage">
    ...
    <section className="workspaceSection attentionQueue"><div className="workspaceSectionHead"><div><span>Attention queue</span><h2>Evidence that changed</h2></div>{unread.length ? <button className="textAction" onClick={() => Promise.all(unread.map((item) => resolve(item.id, "read"))).then(() => undefined)}><CheckCheck/> Mark all read</button> : null}</div>
      <div className="alertRows">{alerts.map((alert) => { const extension = alert.extensions?.[0]; return <article key={alert.id} className={`monitorAlert ${alert.state} severity-${String(alert.severity || "INFORMATIONAL").toLowerCase()}`}><span className="alertSeverity">{alert.severity || "INFORMATIONAL"}</span><span className="alertExtensionIcon"><ExtensionIcon iconUrl={extension?.icon_url} publisher={alert.extension_id} name={extension?.display_name || alert.extension_id} size="logo"/></span><div><span>{labelFor(alert.kind)}</span><h3>{alert.title}</h3><p>{alert.summary}</p><small>{formatDate(alert.created_at)} · {extension?.display_name || alert.extension_id}@{alert.version}</small></div><div className="alertActions"><Link href={`/extensions/${encodeURIComponent(alert.extension_id)}/versions/${encodeURIComponent(alert.version)}`}>Open evidence <ArrowRight/></Link>{alert.state !== "acknowledged" ? <button onClick={() => void resolve(alert.id, "acknowledged")}>Acknowledge</button> : <span>ACKNOWLEDGED</span>}<button onClick={() => void resolve(alert.id, "dismissed")}>Dismiss</button></div></article>; })}{state === "ready" && !alerts.length ? <Empty title="Nothing needs attention" detail="Watch an extension to receive a release event, then a follow-up scan result with its exact evidence." href="/catalog"/> : null}</div>
    </section>
