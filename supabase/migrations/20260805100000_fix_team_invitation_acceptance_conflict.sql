-- The function returns a table with a `team_id` output variable. Qualifying
-- the upsert by its primary-key constraint avoids PL/pgSQL resolving that name
-- ambiguously when an invitation is accepted.
...
begin
  ...
  if invitation.accepted_at is not null then raise exception 'Invitation has already been accepted.'; end if;
