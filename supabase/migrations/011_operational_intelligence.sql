-- Auditable public intelligence counts. Operational scans, benchmark scans and
-- development regressions deliberately remain separate so marketing cannot
-- accidentally count test fixtures as ecosystem coverage.
...
returns table (
  indexed_extensions bigint,
  exact_releases_indexed bigint,
  exact_releases_analyzed bigint,
  analyzer_complete_reports bigint,
  known_bad_artifacts bigint,
  block_decisions bigint,
  high_risk_reviews bigint,
  latency_sample_size bigint,
...
as $$
  ...
  select
    ...
    (select count(distinct (extension_id, version, artifact_sha256)) from operational where decision <> 'incomplete'),
