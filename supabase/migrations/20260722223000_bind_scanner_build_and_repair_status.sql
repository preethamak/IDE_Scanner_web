...
do $$ begin
  alter table public.scan_jobs
    ...
    check (expected_scanner_build is null or expected_scanner_build ~ '^[0-9a-f]{40}$');
...
begin
  ...
  if p_job_id is not null then
    ...
    where id = p_job_id and profile = 'deep' and status = 'queued'
      and expected_scanner_build = p_scanner_build
...
-- Correct only the additive status projection for historical rows. Decisions,
-- severities, findings, reports, and scanner identities remain immutable.
update public.scans
set analysis_status = case
  when analysis_coverage #>> '{manifest_validation,status}' = 'scan-aborted'
    or analysis_coverage #>> '{providers,artifact_acquisition,status}' = 'failed'
    then 'failed'
  when analysis_coverage ->> 'status' = 'complete'
