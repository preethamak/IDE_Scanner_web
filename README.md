...
- Quick Scan runs bounded hosted-static analysis and never claims Semgrep, YARA, or dependency coverage.
...
- Supabase Free provides Postgres, Auth, RLS-protected personal data, and team workflow data.
...
SUPABASE_SERVICE_ROLE_KEY
SCANNER_WORKFLOW_TOKEN
```

The service role and workflow token are server-only. Never expose them through `NEXT_PUBLIC_*` variables. Deep Scan callbacks are HMAC-verified before schema validation and ingestion.
